 ...
 * @link     https://accolade.fi
 */
class Accolade_Button_CustomerController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function deleteAction()
    {
        ...
        if (isset($params['id'])) {
            $error = Mage::getModel('accolade_button/button')
                ->setCustomerButton($customer, $params['id']);
        }
        if ($error == 0) {
            $button = $customer->getButton();
            $response = Mage::helper('accolade_button/api')
                ->releaseButton($button->getAssociationId());
            if (gettype($response) == 'array') {
                $button->delete();
                Mage::getSingleton('core/session')
                    ->addSuccess(
                        Mage::helper('core')
                        ->quoteEscape($this->__('Button successfully deleted'))
                    );
            } else {
                if (gettype($response) == 'object' 
                    ...
                    Mage::getSingleton('core/session')
                        ->addError($this->__('Button API Error'));
                }
            }
        }
        $this->_forward('index');
    ...
    {
        $newButton = Mage::getModel('accolade_button/button');
        Mage::getSingleton('customer/session')
            ->setButton($newButton);
        $this->_forward('edit');
    }
    ...
    {
        $error = 0;
        $checkShippingAddress = Mage::getModel('accolade_button/button')
        ...
        $apiHelper = Mage::helper('accolade_button/api');
        $model = Mage::getSingleton('customer/session')->getButton();
        $new = false;
        ...
        if (empty($model)) {
            ...
            $values = array(
                "button_id" => array(
                    ...
                    "error_message" => $this->__('Invalid Button device ID!'),
                    "api_call" => function ($value) use ($apiHelper) {
                        return $apiHelper->associateButton($value);
                    }
                ),
            ...
            if (count($data) > 0) {
                try {
                    $data['customer_id'] = Mage::getModel('accolade_button/button')
                        ->getCustomerId();
                    $model->addData($data);
                    $model->save();
                } catch (Exception $e){
                    $error = 1;
                    Mage::log($e->getMessage(), null, 'button-error.log', true);
                }
                if ($error == 0) {
                    Mage::getSingleton('core/session')
                        ->addSuccess('Button settings successfully updated!');
                }
            }
        }
        Mage::getSingleton('customer/session')
            ->setButton(Mage::getModel('accolade_button/button'));
